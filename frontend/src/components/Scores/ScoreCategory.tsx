...
export function ScoreCategory({ category, isExpanded, onToggle }: ScoreCategoryProps) {
  ...
  const weightLabel = category.weight
    ? category.weight.label ?? formatDisplayPercentage(category.weight.value, category.weight.maxValue)
    : undefined;
      ...
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="flex shrink-0 items-center gap-3">
          <span className="text-sm font-black text-slate-950 dark:text-white">
            {showScore ? formatDisplayPercentage(category.riskScore, category.maxScore) : 'Unknown'}
          </span>
          <span
            aria-hidden="true"
