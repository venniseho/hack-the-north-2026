...
export default function App() {
  ...
  const [lastRequest, setLastRequest] = useState<AnalysisRequest>({
    mode: "standard",
  });
  ...
  async function runAnalysis(request: AnalysisRequest = { mode: "standard" }) {
    ...
    try {
      ...
      const instagramLinks =
        ...
        (request.currentUrl === undefined
          ? await collectInstagramLinks(activeTab?.id)
          : []);
    ...
    <Panel theme={theme}>
      ...
      <main className="min-h-0 flex-1 overflow-y-auto">
        ...
        {viewState === "error" && (
          <ErrorState
            message={error}
            onRetry={() => void runAnalysis(lastRequest)}
          />
        )}
    ...
    <div className="px-4 py-5">
      <section className="rounded-md border border-slate-200 bg-white p-4 shadow-sm dark:border-teal-700/60 dark:bg-[#082a31]">
        ...
        <p className="mt-2 text-xs leading-5 text-slate-600 dark:text-teal-100/70">
          Run a compact risk review for suspicious reputation and review-quality
          signals.
        </p>
